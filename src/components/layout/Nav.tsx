...
export default function Nav() {
  ...
  return (
    <>
      ...
      <div
        className={`md:hidden fixed top-16 right-0 bottom-0 z-40 w-72 bg-surface border-l border-border transition-transform duration-300 ${
          open ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="font-mono text-[7px] tracking-[0.25em] text-ink-30 px-7 pt-8 pb-4">
          NAVIGATION
        </div>
        <nav>
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                aria-current={active ? 'page' : undefined}
                className={`flex items-center px-7 h-14 font-serif text-[16px] border-b border-border transition-colors ${
                  active
                    ? 'text-ink bg-base'
                    : 'text-ink-60 hover:text-ink hover:bg-base'
                }`}
              >
                <span className={`w-1 h-1 rounded-full mr-3 ${active ? 'bg-accent' : 'bg-border'}`} />
                {link.label}
              </Link>
            )
          })}
