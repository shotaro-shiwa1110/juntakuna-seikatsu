...
export default function LogCard({ entry }: Props) {
  return (
    <Link href={`/log/${entry.slug}`} className="group block border border-border bg-surface hover:bg-base transition-colors">
...
export default function ProjectCard({ project }: Props) {
  return (
    <Link href={`/projects/${project.slug}`} className="group block bg-base border-r border-border last:border-r-0 p-5 hover:bg-surface transition-colors">
  ...
  return (
    ...
    >
      ...
      {edges.map(([x1, y1, x2, y2], i) => (
        <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={i === 4 ? '#B8F000' : '#CCC'} strokeWidth="0.8" />
      ))}
